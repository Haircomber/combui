...
func dump_liq_stack_item(w http.ResponseWriter, key [32]byte) {
	...
	fmt.Fprintf(w, `<h2><tt>%s</tt> %d.%08d COMB</h2>`+"\n", CombAddr(key), combs(bal), nats(bal))
	...
	fmt.Fprintf(w, ` <ul><li><tt>%s</tt>  %d.%08d COMB</li><li><tt>%s</tt> %d.%08d COMB / %d.%08d COMB </li></ul>`+"\n",
		CombAddr(to), combs(balto), nats(balto), CombAddr(sumto), combs(balsumto), nats(balsumto), combs(sum), nats(sum))
...
func stacks_view(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
