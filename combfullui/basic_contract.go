...
func commits_entrenched_height(commitment1, commitment2 [32]byte) uint32 {
	...
	var tag1, ok1 = commitsCheckNoMaxHeight(commitment1)
	var tag2, ok2 = commitsCheckNoMaxHeight(commitment2)
...
func gui_contract(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	fmt.Fprint(w, `
		...
		<div id="buttons" style="float:left;visibility:hidden"><ul style="margin:auto 0;padding:auto 0">
			...
			<li><a href="#" id="doauction">Auction contract</a></li>
		</ul></div>
		...
		<script type="text/javascript">
		  var foo = function() {
		    ...
		    document.getElementById("doauction").href="/basiccontract/auction/"+decider;
		    return false;
		  };
...
func do_decider_common(decider string) (short_decider, left_decider, right_decider, left_decider_tip, right_decider_tip [32]byte, number uint16, err1 error) {
	...
	if len(decider) == 64 {
		...
		short_decider = nethash(buf3_a0[0:])
...
func gui_adl_contract(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {

	fmt.Fprintf(w, testnetColorBody()+`<a href="/basiccontract/">&larr; Back to basic contract</a><br />`)
	defer fmt.Fprintf(w, `</body></html>`)

	var decider_str = ps.ByName("decider")
...
func gui_trade_contract(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
...
func gui_auction_contract(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	fmt.Fprintf(w, `
		...
		<div>Associated short decider: %s</div>
		<div>Merkle branch: <tt id="merklebranch">%d</tt></div>
		<div>Bidder id: <input name='bidder_id' id='bidder_id' /> (1 - 65535)</div>
		<div>Vendor's forward COMB address: <input name='forward_address' id='forward_address' style="font-family:monospace;width:45em" maxlength="64" /></div>
		<div>Bidder's rollback COMB address: <input name='rollback_address' id='rollback_address' style="font-family:monospace;width:45em" maxlength="64" /></div>
	`, decider_str, Hexpand32(short_decider[0:]), uint16_reverse(num))
	...
	fmt.Fprintf(w, `
		...
		<script type="text/javascript">
		  var foo = function() {
		    ...
		    var forward_address = document.getElementById("forward_address").value;
		    var rollback_address = document.getElementById("rollback_address").value;
                    bidder_id = parseInt(bidder_id).toString(16).toUpperCase().padStart(4, "0");

		    document.getElementById("ok").href="/basiccontract/auction/%s/"+
			bidder_id+"/"+forward_address+"/"+rollback_address;
		    return false;
		  };
		document.getElementById("bidder_id").oninput = foo;
		document.getElementById("bidder_id").onpropertychange = foo;
		document.getElementById("forward_address").oninput = foo;
		document.getElementById("forward_address").onpropertychange = foo;
		document.getElementById("rollback_address").oninput = foo;
		document.getElementById("rollback_address").onpropertychange = foo;
...
func gui_adl_contract_merkle(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	var min_str, max_str, left_str, right_str = ps.ByName("min"), ps.ByName("max"), ps.ByName("left"), ps.ByName("right")
	...
	err4 := checkHEX8upper(min_str)
	if err4 != nil {
		fmt.Fprintf(w, "error: min amount invalid: %s", err4)
		return
	}

	err5 := checkHEX8upper(max_str)
	if err5 != nil {
	...
	fmt.Fprintf(w, `
		...
		<div>Change COMB address: <tt name='left_address' id='left_address' style="font-family:monospace;width:45em">%s</tt></div>
		<div>Exact COMB address: <tt name='right_address' id='right_address' style="font-family:monospace;width:45em">%s</tt></div>
		<div>Merkle root: <tt id="merkleroot">%x</tt></div>
		<div>Address: <tt id="address">%s</tt></div>
	`, decider_str, Hexpand32(short_decider[0:]), combs(min), nats(min), combs(max), nats(max), CombAddr(left), CombAddr(right), branch[17], CombAddr(address))
...
func gui_trade_contract_merkle(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	var trade_str, forward_str, rollback_str = ps.ByName("trade"), ps.ByName("forward"), ps.ByName("rollback")
	...
	fmt.Fprintf(w, `
		...
		<div>Forward COMB address: <tt name='forward_address' id='forward_address' style="font-family:monospace;width:45em">%s</tt></div>
		<div>Rollback COMB address: <tt name='rollback_address' id='rollback_address' style="font-family:monospace;width:45em">%s</tt></div>
		<div>Merkle root: <tt id="merkleroot">%x</tt></div>
		<div>Address: <tt id="address">%s</tt></div>
	`, decider_str, Hexpand32(short_decider[0:]), trade, CombAddr(forward), CombAddr(rollback), branch[17], CombAddr(address))
...
func gui_auction_contract_merkle(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	var bidder_id, forward_str, rollback_str = ps.ByName("bidderid"), ps.ByName("forward"), ps.ByName("rollback")
	...
	err4 := checkHEX2upper(bidder_id)
	if err4 != nil {
		fmt.Fprintf(w, "error: bidder id invalid: %s", err4)
		return
	}

	forward := hex2byte32([]byte(forward_str))
	rollback := hex2byte32([]byte(rollback_str))
	bidder := hex2uint16([]byte(bidder_id))

	var branch = auction_generate_branch(uint16_reverse(bidder), num, forward, rollback)
	...
	fmt.Fprintf(w, `
		...
		<div>Decider: <font size="3"><tt id="decider">%s</tt></font></div>
		<div>Associated short decider: %s</div>
		<div>Bidder ID: <tt name='trade_bit' id='trade_bit'>%d</tt></div>
		<div>Vendor's forward COMB address: <tt name='forward_address' id='forward_address' style="font-family:monospace;width:45em">%s</tt></div>
		<div>Bidder's rollback COMB address: <tt name='rollback_address' id='rollback_address' style="font-family:monospace;width:45em">%s</tt></div>
		<div>Merkle root: <tt id="merkleroot">%x</tt></div>
		<div>Address: <tt id="address">%s</tt></div>
	`, decider_str, Hexpand32(short_decider[0:]), bidder, CombAddr(forward), CombAddr(rollback), branch[17], CombAddr(address))
...
func auction_generate_branch(trade_bit uint16, branch_id uint16, forward_address, rollback_address [32]byte) [18][32]byte {
	var buf [65536][32]byte
	for i := range buf {
		if uint16(i) == trade_bit {
			buf[i] = forward_address
		} else {
			buf[i] = rollback_address
		}
