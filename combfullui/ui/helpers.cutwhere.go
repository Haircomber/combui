...
func mul(a *[BIG + 1]uint32, c *[BIG + 1]uint32, m uint32) {
	var d uint64
	for i := 0; i < BIG+1; i++ {
		d += uint64(a[i]) * uint64(m)
		c[i] = uint32(d)
		d >>= 32
	}
}

func div(a *[BIG + 1]uint32, c *[BIG + 1]uint32, m uint32) uint32 {
	var d uint64
	var r uint64
	for i := BIG + 1; i > 0; i-- {
		d = (r<<32 + uint64(a[i-1])) / uint64(m)
		r = (r<<32 + uint64(a[i-1])) % uint64(m)
		c[i-1] = uint32(d)
	}
	return uint32(r)
}

func CutCombWhere2(m []byte) (out [CHAINS]uint16) {
	var n = uint64(LEVELS)
	var x [BIG]uint32
	for i := 0; i < 32; i++ {
	...
	out[CHAINS-1] = uint16(n)
	return out
}

func Z(n, c uint64) (out [BIG]uint32) {
	if c <= 1 {
		out[0] = 1
		return
	}
	var o [BIG + 1]uint32
	o[0] = 1
	for i := n + c - 1; i > (n) && i > (c-1); i-- {
		mul(&o, &o, uint32(i))
	}
	for i := (uint64)(2); i <= (n) && i <= (c-1); i++ {
		if div(&o, &o, uint32(i)) != 0 {
			println(n, c)
			panic("remainder is nonzero")
		}
	}
	if o[BIG] != 0 {
		println(n, c)
...
func CutComb(hex string) [CHAINS]uint16 {
	var buf [32]byte
	for i := range buf {
		buf[i] = (x2b(hex[i<<1]) << 4) | x2b(hex[i<<1|1])
	}
	return CutCombWhere2(buf[:])
}
