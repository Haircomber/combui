...
func decider_key_to_short_decider(key [64]byte) (o [32]byte) {
	...
	for i := uint32(0); i < 65535; i++ {
		o = nethash(buf3_a0[32:64])
		copy(buf3_a0[32:64], o[0:])
		o = nethash(buf3_a0[64:96])
...
func purse_generate_key(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {

	var key [64]byte

	fmt.Fprintf(w, testnetColorBody()+`<a href="/purse/">&larr; Back to deciders purse</a><br />`)
...
func purse_browse(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
...
func sign_use_decider(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	fmt.Fprintf(w, testnetColorBody()+`<a href="/purse/">&larr; Back to purse</a><br />`)
	defer fmt.Fprintf(w, `</body></html>`)

	var pursekey, destination = ps.ByName("decider"), ps.ByName("number")
	...
	err2 := checkHEX2upper(destination)
	if err2 != nil {
