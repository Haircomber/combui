...
func stack_load_data(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
...
func stack_load_data_internal(w http.ResponseWriter, data string) [32]byte {
	...
	if w != nil {
		fmt.Fprintf(w, "loaded stack address: <tt>%s</tt>", CombAddr(hash))
	}
	commits_mutex.RLock()
