...
func testnetColorBody(args ...string) string {
	if len(args) > 0 {
		if Testnet() {
			return `<html><head><title>Haircomb Core Wallet - Testnet</title></head><body bgcolor="#AAFFAA" ` + args[0] + `>`
		}
		return `<html><head><title>Haircomb Core Wallet</title></head><body ` + args[0] + `>`
	}

	if Testnet() {
		return `<html><head><title>Haircomb Core Wallet - Testnet</title></head><body bgcolor="#AAFFAA">`
	}
	return `<html><head><title>Haircomb Core Wallet</title></head><body>`
}

var mainPageHash = sha256.Sum256([]byte("/main"))

func mainPageInvalidate() {
	ph := mainPageHash
	GzipUnstorePathHash(ph)
}

func main_page_json(w http.ResponseWriter, r *http.Request) {
	...
	last_btc_height := check_last_known_btc_height()

	var accel, bh, p2wshcount = accelerator_get()
	...
	var data = struct {
		...
		P2WSHCount        uint64
		ShutdownButton    bool
		DecidersHardfork  bool
	}{
		Testnet(),
		height < uint64(fakeheight) || (last_btc_height != -1 && last_btc_height < int(our_top_block().Height)),
		int64(last_btc_height),
		fmt.Sprintf("%x", cryptofingerprint),
		size,
		CombAddr(accel),
		fmt.Sprintf("%x", bh),
		sum,
		...
		true,
		p2wshcount,
		u_config.shutdown_button,
		u_config.deciders_fork,
	}
...
func main_page(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if Testnet() {
		bgColor = ` bgcolor="#AAFFAA"`
		subtitle = " - Testnet"
		subtname = `<sub style="font-size:50%"> Testnet</sub>`
	}

	fmt.Fprintf(w, `
		<html><head>
		<title>Haircomb Core Wallet%s</title>
		...
		<h1 style="padding-top:0; margin-top:0"> Haircomb Core%s</h1></td>
...
`, subtitle, bgColor, subtname)
	...
	var fakeheight = makefaketag().height
	last_btc_height := check_last_known_btc_height()
	if height < uint64(fakeheight) || (last_btc_height != -1 && last_btc_height < int(our_top_block().Height)) {
	...
	var accel, bh, _ = accelerator_get()
	if accel != [32]byte{} {
		fmt.Fprintf(w, "<p>%s Accelerator (do not pay there) as of block %x</p>", CombAddr(accel), bh)
	}
	...
	if u_config.deciders_fork {
		fmt.Fprintf(w, "<p>Deciders Hardfork is <strong>Active</strong></p>")
	} else {
		fmt.Fprintf(w, "<p>Deciders Hardfork is <strong>Inactive</strong></p>")
	}
	fmt.Fprintf(w, "<p><a href=\"/utxo/bisect/b%08d\">&#128272; Validate database integrity with peer</a></p>", height&0xFFFFFC00)
}

func shutdown_page(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if !wallet_flushed_ok {
		...
		fmt.Fprintf(w, testnetColorBody()+`<a href="/">Back</a><h1>Wallet is not saved! You may not shutdown!</h1></body></html>
		`)
		return
	}

	fmt.Fprintf(w, testnetColorBody()+`Shutdown successfull. You may now close the window.</body></html>
	`)
	// Close the DB
	CommitLvlDbClose()
	os.Exit(0)
...
func version_page(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
