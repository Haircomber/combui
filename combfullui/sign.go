...
func sign_use_key(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	fmt.Fprintf(w, testnetColorBody()+`<a href="/wallet/">&larr; Back to wallet</a><br />`)
	defer fmt.Fprintf(w, `</body></html>`)

	var walletkey, destination = ps.ByName("walletkey"), ps.ByName("destination")
	...
	var txid = nethash(txsli)
	depths := CutCombWhere(txid[0:])

	fmt.Fprintf(w, "<h1>txid: %x</h1>\n", txid)
	fmt.Fprintf(w, "<p>To pay to address <tt>%s</tt> follow these steps:</p>\n", CombAddr(dest))
	...
	fmt.Fprintf(w, "<p>2. Pay 330 satoshi each to 21 addresses below:</p>\n")
...
func sign_gui(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	fmt.Fprintf(w, testnetColorBody()+`
		<a href="/wallet/">&larr; Back to wallet</a><br />
