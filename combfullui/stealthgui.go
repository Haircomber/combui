...
func paginator(w http.ResponseWriter, key [32]byte, off uint64) {
	if off == 0 {
		fmt.Fprintf(w, "<center>You are on page %d of 10000000000000000 <a href=\"/wallet/stealth/%s/%016d\">Page %d &gt;</a></center>\n", off+1, CombAddr(key), off+1, off+2)
	} else {
		fmt.Fprintf(w, "<center><a href=\"/wallet/stealth/%s/%016d\">&lt; Page %d </a> You are on page %d of 10000000000000000 <a href=\"/wallet/stealth/%s/%016d\">Page %d &gt;</a></center>\n", CombAddr(key), off-1, off, off+1, CombAddr(key), off+1, off+2)
	}
}

func stack_load_stealth_data(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	fmt.Fprintf(w, `<a href="/wallet/stealth/%s/%016d">&larr; Back to your stealth addresses</a><br />`, CombAddr(key), off)

}

func wallet_stealth_view(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {

	var backingkey, offset = ps.ByName("backingkey"), ps.ByName("offset")
	...
	fmt.Fprint(w, testnetColorBody()+`<a href="/wallet/">&larr; Back to wallet</a><br />`)
	defer fmt.Fprintf(w, `</body></html>`)

	fmt.Fprintf(w, "<h1>Stealth addresses redirecting to: <tt>%s</tt></h1>\n", CombAddr(key))

	fmt.Fprintf(w, `<h2>Any Bitcoin send to mine address will be LOST FOREVER, however haircomb will be generated.</h2> `+"\n")
	fmt.Fprintf(w, `<p>Only pay 330 satoshi to mine address. The more times you pay to different Bitcoin mine addresses the bigger your chance to win a free haircomb.</p> `+"\n")
	...
	for i := uint16(0); i < 256; i++ {
		...
		commits_mutex.RLock()
		basetag, is_commited := commitsCheckNoMaxHeight(ckey)
		...
		fmt.Fprintf(w, `<ul><li> <tt>%s</tt> %d.%08d COMB `, CombAddr(addr), combs(bal), nats(bal))
